// dashboard js operations
const dropdownButton = document.querySelector('.dropdown > i')
dropdownMenu = document.querySelector('.dropdown_menu');

// when the dropdown
// button is clicked
dropdownButton.onclick = function() {
	if(dropdownButton.classList.contains('fa-angle-down')) {
		dropdownMenu.style.transform = 'translate(0px, 5px) scale(1)';
		dropdownButton.classList.remove('fa-angle-down');
		dropdownButton.classList.add('fa-angle-up');
	} else {
		dropdownMenu.style.transform = 'translate(0px, 0px) scale(0)';
